...
\begin{figure}\caption{Policy Functions for Homeownership and Savings}\label{fig:pol}
	...
	\end{center}
	{\begin{footnotesize} \textit{Notes:} The dashed vertical lines indicate the threshold where the kid switches from renting to owning. The horizontal dotted lines indicate the PMI and LTV thresholds. The child states $\mathbf{s}_k'$ are \input{../tabfig/model/pol_bond_state.tex}\unskip. The figure plots the policy functions in the estimated model. I discuss the estimation in Section \ref{sec:estimation}. The qualitative patterns remain consistent across parameterizations. \end{footnotesize}}
\end{figure}
...
Finally, the function $\mathcal{H}$ operates on the distribution $\psi(\mathbf{s}_p)$ and policy functions %$g^*(\mathbf{s}_p):\mathbf{S}_p \rightarrow (X\times B \times T \times H)\times (X\times B \times H) \times (A_k)$ 
