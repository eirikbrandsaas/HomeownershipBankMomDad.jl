...
% <=100 word abstract This paper quantifies how parental transfers shape young adults’ homeownership. I estimate a life-cycle OLG model with housing in which parents and children interact without commitment. Parental transfers account for 13 percentage points (27\%) of young households’ homeownership. Transfers from wealthy parents both relax borrowing constraints and sustain ownership by mitigating housing illiquidity. Policy experiments show that easing entry barriers increases reliance on parental wealth, whereas increasing housing liquidity reduces it. Finally, I show that children of wealthy parents strategically use the illiquidity of housing as a commitment device to encourage transfers, resulting in a preference for illiquidity.
\end{spacing}}
\vspace{0.5cm}
...
I first present time trends in parental transfers for down payments, before describing the estimation sample. Finally, I use the estimation sample to show that parental wealth is positively associated with better housing outcomes, both before and after purchase.
...
\begin{figure}
	...
	\caption*{\begin{footnotesize} \textit{Notes:}  Left panel (SHED): first-time owners by purchase year; outcome is the share receiving a down-payment transfer. Right panel (AHS): owner-occupied units without prior-sale proceeds by survey year; outcome is the share whose down payment was primarily funded by a transfer. Higher levels in the SHED reflect its broader definition. Dashed lines denote 95\% confidence intervals.\end{footnotesize}}
\end{figure}
...
% \cleardoublepage  % This ensures you start on a new page
% \thispagestyle{empty}  % This removes page number and headers/footers
% \vspace*{\fill}  % This pushes the text to the vertical center of the page
% \begin{center}
% \Large\textbf{Online Appendix}  % This creates the text, centered and bold
% \end{center}
% \vspace*{\fill}  % This completes the vertical centering
% \clearpage  % This ends the current page
\appendix
...
\begin{figure}
	...
	\noindent%
	\includegraphics[width=0.45\textwidth]{../tabfig/descr/PSID_housinggrowthpoorall_both}%
	\includegraphics[width=0.45\textwidth]{../tabfig/descr/PSID_housinggrowthrichall_both}
