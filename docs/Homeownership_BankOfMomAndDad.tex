...
\begin{equation}	\label{eq:regmaintain}
	...
	= \beta\ln(NetWorth_{i,t}) + \zeta\ln(ParWealth_{i,t})
	  + \gamma' X_{i,t} + \varepsilon_{i,t},
\end{equation}
where $Own_{i,t+\Delta}=1$ denotes an owner at time $t+\Delta$, $\beta$ and $\zeta$ measures the effect of log household net worth and parental wealth, respectively  and $X_{i,t}$ is a vector of control variables. The only changes relative to Eq. \ref{eq:transferrent} is that the outcome variable conditions on current ownership and measures future ownership, that there are no controls for transfer receipt, and that I have taken the two variables of interest out of the vector of control variables. The sample is limited to households aged 25 to 44. These regressions are motivated by \cite{bond2021role}, who estimate similar equations in the PSID and the Health and Retirement Study. Figures \ref{fig:coef_allowner} and \ref{fig:coef_ftowner} plots the two coefficients of interest over different time horizons.
...
\end{equation*}
using the same control variables as in the previous regression. Table~\ref{tab:hypo} reports the estimated coefficients on household net worth ($\beta$); full results appear in Appendix Table~\ref{tab:hypo_long}.  
